...
// GET /api/agent/blog/[id] - Get a blog post by ID
...
// PUT /api/agent/blog/[id] - Update a blog post (resubmit for approval)
export async function PUT(
  ...
  try {
    ...
    // Check post exists and belongs to this agent
    ...
    // Agents can only edit draft or rejected posts
    if (existing.status === BlogPostStatus.PUBLISHED) {
      return NextResponse.json(
        { error: "Published posts cannot be edited. Contact admin for changes." },
        { status: 400 }
      )
    }
...
export async function DELETE(
  ...
  try {
    ...
    if (existing.status === BlogPostStatus.PUBLISHED) {
      return NextResponse.json(
        { error: "Published posts cannot be deleted. Contact admin." },
        { status: 400 }
      )
    }
